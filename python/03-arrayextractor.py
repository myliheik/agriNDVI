...
2022-11-30 MY # to exclude nodata values : modified dataset.read # nodata value on 64537
...
NO_DATA = np.nan # for filling 
NO_DATA_IN_RASTER = -999
INVALID = 0
...
def main():
    ...
    if not os.path.exists(projectname):
        print(f'Creating output directory {projectname}...')
    ...
    if os.path.isfile(shapefile + '.shp'):
        ...
        print(f'Tile {tile} does not have any parcels. Exiting now.')
...
def extractarray(raster_path,
    ...
    for band_path in band_paths:
        ...
        for parcel_band_raster, parcel_cloud_mask in zip(band_raster_per_parcel, parcel_cloud_masks):
            filtered_band: np.ndarray = filter_band_using_mask(parcel_band_raster, parcel_cloud_mask).astype(np.uint16)
...
def filter_band_using_mask(parcel_band_raster: [dict], parcel_cloud_mask: [dict]):
    ...
    parcel_array = parcel_band_raster['properties']['mini_raster_array'].astype('float64')
    parcel_array = parcel_array.filled(NO_DATA)
    ...
    return filtered_array[np.isfinite(filtered_array)]
